    ...
    <div x-show="show" class="fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto">
        ...
        <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
            ...
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ $profileUser->display_name ?? '' }}'s {{ __('profile.following_modal_title') }}</h2>
            ...
            @if ($following->isEmpty())
                <p class="text-gray-600 dark:text-gray-400">{{ __('profile.not_following_anyone_publicly') }}</p>
            @else
                <ul class="divide-y divide-gray-200 dark:divide-zinc-700 max-h-80 overflow-y-auto">
                    @foreach ($following as $followedUser)
                        <li class="py-3 flex items-center space-x-3">
                            <img class="h-10 w-10 rounded-full object-cover" src="{{ $followedUser->avatar }}" alt="{{ $followedUser->display_name }}">
                            <div>
                                <a href="{{ $followedUser->profile_link }}" wire:navigate class="text-gray-900 dark:text-gray-100 font-medium hover:underline">{{ $followedUser->display_name }}</a>
                                @if ($followedUser->handle)
                                    <p class="text-gray-500 dark:text-gray-400 text-sm">@{{ $followedUser->handle }}</p>
                                @endif
                            </div>
                        </li>
                    @endforeach
